...
- Capped HTTP(S) request bodies at 4 MiB so a large window no longer demands
  ...
  delivering bytes licenses more resume attempts than the flat three-attempt
  rule.
- Surface the media source's own error (HTTP status, timeout) in demux errors
  rather than a bare `Input/output error (-5)`.
- Reads that end far beyond the stream window (a demuxer probing the tail, a
  large forward seek) no longer wait out the full stream-stall clock before
  re-anchoring: the workers stop at the read-ahead boundary, so the wait could
  never succeed. They fall straight through to the synchronous re-anchor,
  cutting fixed ~20 s stalls on open and post-seek buffering over fast sources.
